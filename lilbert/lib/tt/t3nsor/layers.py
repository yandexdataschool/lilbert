...
from lib.tt import t3nsor as t3
...
class TTConv1(TTLinear):

    def forward(self, x):
        weight_t = self.weight_t
        x_t = x.transpose(0, 2)
        if self.bias is None:
            return t3.tt_dense_matmul3(weight_t, x_t).transpose(0, 2)
        else:
            return t3.tt_dense_matmul3(weight_t, x_t).transpose(0, 2) + self.bias
